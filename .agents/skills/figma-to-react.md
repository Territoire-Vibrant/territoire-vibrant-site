---
alwaysApply: false
description: Coding standards and prompt for converting designs into React 19 components using Tailwind CSS v4, Base UI, and shadcn/ui architecture (CVA + cn utility).
globs: "**/*.tsx"
---
...
- **Class Variance Authority** (`class-variance-authority` / `cva`) for variants
- **Tailwind Merge & clsx** (via standard `cn` utility) for class merging
...
- Assume the utility `cn` is available at `@/lib/utils`
...
export const buttonVariants = cva(
    'inline-flex cursor-pointer items-center justify-center font-medium rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
    {
        variants: {
            variant: {
            ...
            size: {
                ...
                lg: 'h-9 px-4 gap-2.5 text-base [&_svg]:size-3.5',
            },
        },
        defaultVariants: {
            variant: 'primary',
...
export const Button = ({ className, variant, size, disabled, children, ...props }: ButtonProps) => {
    return (
        <button
            type="button"
            data-slot="button"
...
// Combine variants and className directly in cn()
className={cn(buttonVariants({ variant, size, className }))}
...
<button aria-label="Close"><X className="size-3.5" /></button>
